...
- **Authentification** — protection par mot de passe et cookie de session ; premier accès invitant à définir un mot de passe ; limitation après 10 échecs de connexion (verrouillage de 30 minutes)
...
Ouvrez `http://localhost:53136` dans votre navigateur. Lors de la première visite, vous serez invité à définir un mot de passe — celui-ci protège le tableau de bord et tous les endpoints de l'API. Une fois le mot de passe défini, vous êtes automatiquement connecté. Ajoutez des serveurs via l'interface.
