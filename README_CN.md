...
xattr -d com.apple.quarantine belochka-darwin-arm64  # 清除 Gatekeeper 隔离标记
...
```bash
make release
# 输出（仅 Linux + Windows；macOS 二进制文件仅由 CI 产出）：
...
| `BELOCHKA_LOG_RETENTION_DAYS` | 日志文件中保留日志条目的天数（默认：`3`）|
