...
// fakePasswordStore implements PasswordStore for testing.
type fakePasswordStore struct {
	hash string
}

func (f *fakePasswordStore) PasswordHash() string         { return f.hash }
func (f *fakePasswordStore) SetPasswordHash(h string) error { f.hash = h; return nil }
...
func TestSetupPassword(t *testing.T) {
	...
	err := store.SetupPassword("secret123", "secret123")
	if err != nil {
...
func TestSetupPasswordMismatch(t *testing.T) {
	...
	err := store.SetupPassword("secret123", "different")
...
func TestHandleSetup(t *testing.T) {
	...
	w2 := httptest.NewRecorder()
	h.HandleSetup(w2, postJSON("/api/setup", `{"password":"another","confirm_password":"another"}`))
...
func TestHandleSetupMismatch(t *testing.T) {
	...
	w := httptest.NewRecorder()
	h.HandleSetup(w, postJSON("/api/setup", `{"password":"secret123","confirm_password":"different"}`))
...
func TestFullFlowIntegration(t *testing.T) {
	...
	// 2. Setup password.
	w := httptest.NewRecorder()
	handler.HandleSetup(w, postJSON("/api/setup", `{"password":"mypassword","confirm_password":"mypassword"}`))
	if w.Code != http.StatusOK {
